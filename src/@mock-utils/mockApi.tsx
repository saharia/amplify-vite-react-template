...
import mockDbJson from './mockDb.json';


// Clone it into a mutable object
const mockDb: { [key: string]: any } = JSON.parse(JSON.stringify(mockDbJson));
...
const mockApi = (tableName: string) => ({
	...
	async find<T extends { id?: string }>(param: string | Record<string, unknown>) {
		...
		return table.find((item) => Object.entries(param).every(([key, value]) => (item as Record<string, unknown>)[key] === value)) || null;
	},
