...
const FirebaseAuthProvider = forwardRef<FuseAuthProviderMethods, FuseAuthProviderComponentProps>(
  ({ children, onAuthStateChanged }, _) => {
				...
				if (firebaseUser) {
					...
					try {
						...
						if (!userAttributes || !userAttributes.email) {
							throw new Error('User attributes or email is null');
						}
						...
						const newUserResponse = await authCreateDbUser({
							...
							photoURL: userAttributes?.photoURL ?? undefined
						});
	 ...
	 */
	const signIn: FirebaseAuthContextType['signIn'] = useCallback(({ email, password }: FirebaseSignInPayload) => {
		try {
	 ...
	 */
	const signUp: FirebaseAuthContextType['signUp'] = useCallback(async ({ email, password }: FirebaseSignInPayload) => {
		try {
