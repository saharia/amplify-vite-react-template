...
function SchemePreview(props: SchemePreviewProps) {
	...
	const primaryColor: string = theme.palette.primary?.[500 as unknown as keyof PaletteColor] || theme.palette.primary?.main || '#000000';
	const primaryColorContrast = theme.palette.primary?.contrastText || _theme.palette.getContrastText(primaryColor);

	const secondaryColor: string = theme.palette.secondary?.[500 as unknown as keyof PaletteColor] || theme.palette.secondary?.main || '#000000';
	const secondaryColorContrast =
		theme.palette.secondary?.contrastText || _theme.palette.getContrastText(secondaryColor);
	const backgroundColor = theme.palette.background?.default || '#ffffff';
	const backgroundColorContrast = _theme.palette.getContrastText(backgroundColor);
	const paperColor = theme.palette.background?.paper || '#ffffff';
	const paperColorContrast = _theme.palette.getContrastText(paperColor);
