...
class FuseUtils {
	...
	static setRoutes(
		...
		const applyAuth = (route: FuseRouteItemType, parentAuth: string[] | null) => {
			const auth = route.auth !== undefined ? route.auth : parentAuth || undefined;
			...
			if (route.children) {
				newRoute.children = route.children.map((childRoute) => applyAuth(childRoute, auth ?? null));
			}
		...
		routes = routes.map((route) => {
			const auth = config.auth !== undefined ? config.auth : defaultAuth;
			return applyAuth(route, auth ?? null);
