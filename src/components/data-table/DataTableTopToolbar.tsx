		...
		<div className="flex flex-col w-full py-1 px-3 border-b-1">
			<Box
				...
				ref={(ref: HTMLDivElement) => {
					if (topToolbarRef && 'current' in topToolbarRef) {
						(topToolbarRef as React.MutableRefObject<HTMLDivElement | null>).current = ref;
					}

					if (toolbarProps?.ref && 'current' in toolbarProps.ref) {
						(toolbarProps.ref as React.MutableRefObject<HTMLDivElement | null>).current = ref;
					}
				}}
				sx={[
					(theme) => ({
