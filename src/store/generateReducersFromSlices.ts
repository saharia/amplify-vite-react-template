...
export const generateReducersFromSlices = <T = unknown>(slices: SlicesType): ReducersMapObject<T> => {
	...
	slices?.forEach((slice) => {
		...
		if (secondary) {
			...
			((reducerGroups[primary] as Record<string, Reducer>))[secondary] = slice.reducer;
		} else {
			reducerGroups[primary] = slice.reducer;
		}
	});

	const combinedReducers: Record<string, Reducer | ReducersMapObject> = {};
	...
	Object.entries(reducerGroups).forEach(([key, reducerGroup]) => {
		if (typeof reducerGroup === 'function') {
			...
			combinedReducers[key] = combineReducers(reducerGroup) as Reducer;
		}
	});
